...
#define Image_One_Height (self.width - 24 *Scale_Width) / 2

#define Image_More_Height (self.width - 44 *Scale_Width) / 9 *2

@interface CommunityBigImageCell()
...
@implementation CommunityBigImageCell
...
- (void) initSubViews {
    ...
    self.bigImageView.backgroundColor = BackGrayColor;
...
- (void) layoutSubviews {
    ...
    self.bigImageView.frame = CGRectMake(12 *Scale_Width, self.contentLabel.y + self.contentLabel.height + 10 *Scale_Height, self.width - 24 *Scale_Width, Image_One_Height);
    
    self.otherBackView.frame = CGRectMake(12 *Scale_Width, self.bigImageView.y + self.bigImageView.height, self.width - 24 *Scale_Width, 60 *Scale_Height);
...
- (void) loadContent {
    ...
    if (model.imageArray.count > 0) {
        ...
        NSString *downloudImageUrlStr = model.imageArray[0];
