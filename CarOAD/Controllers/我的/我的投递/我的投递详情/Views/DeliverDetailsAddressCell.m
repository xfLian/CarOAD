...
@interface DeliverDetailsAddressCell()
...
@implementation DeliverDetailsAddressCell
...
- (void)buildSubview {
    ...
    self.iconImageView.image       = [UIImage imageNamed:@"location_address"];
...
- (void)layoutSubviews {
    ...
    self.iconImageView.frame = CGRectMake(15 *Scale_Width, 0, 16 *Scale_Width, self.contentView.height);
...
- (void)changeState {
    ...
    if (adapter.cellType == kDeliverDetailsAddressCellNoDataType) {
...
+ (CGFloat)cellHeightWithData:(id)data {
    ...
    if (model) {
        ...
        CGFloat numberStringWidth = [[model getDistance] widthWithStringFont:UIFont_13];
        
        CGFloat totalStringHeight = [model.address heightWithStringFont:UIFont_14 fixedWidth:Screen_Width - 75 *Scale_Width - numberStringWidth];
        
        // Expend string height.
        if (totalStringHeight > 0) {
            
            model.normalStringHeight = 20 *Scale_Height + totalStringHeight;
            
        } else {
            
            model.normalStringHeight = 40 *Scale_Height;
